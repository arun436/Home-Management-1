...
function EditDocument({data1}) {
  ...
  const planperiods=[3,6,9,12]
	...
	useEffect(()=> {
		axios.get('/api/getdocument/'+data1)
		.then(res => {
			setData({...data, documentname: res.data.Result[0][0].DocumentName,
				startdate: res.data.Result[0][0].StartDate,
				renewalperiod: res.data.Result[0][0].RenewalPeriod,
  ...
  const handleplanperiod=(e)=>{
	...
	const handleSubmit = (event) => {
		...
		axios.put('/api/updatedocument/'+data1, data)
		.then(res => {
  ...
  return (
    ...
    <div className="col-12">
        <label htmlFor="imputDocumentname" className="form-label">DocumenttName</label>
        <input type="text" className="form-control" id="imputDocumentname"  name="imputDocumentname" placeholder='Select Document Name' autoComplete='off'
     ...
     defaultValue={data?.startdate|| ''}/>
      ...
      <div className="col-12">
			...
			<select className="form-control form-control-lg-col-12" name='planperiod' value={data.renewalperiod}
